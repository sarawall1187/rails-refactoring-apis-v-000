class GithubService
  ...
  def get_repos
    ...
    # binding.pry
    JSON.parse(repos.body)['name']
  end

  def create_repo
      response = Faraday.post "https://api.github.com/user/repos", {name: params[:name]}.to_json, {'Authorization' => "token #{@access_token}", 'Accept' => 'application/json'}
      binding.pry
